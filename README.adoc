...
. Create user and group `{remote-user}`:
+
[source, subs="attributes"]
----
useradd --no-create-home --shell=/bin/sh --user-group {remote-user}

# or if you don' have useradd
addgroup {remote-user}
adduser -h {remote-dir} -s /bin/sh -G {remote-user} -D -H {remote-user}
