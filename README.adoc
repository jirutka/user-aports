...
:repos-uri: https://alpine.geeknet.cz/packages
...
* {repos-uri}/{gh-branch}/legacy

Old versions of some aports that may be installed and used together with their latest version from the official repository.
This is equivalent to https://github.com/Homebrew/homebrew-versions[homebrew-versions] or like poor man’s Gentoo slots.

Binaries are installed into `/usr/bin` as usual, but with a version suffix, e.g. `ruby2.1`.


=== Main

* {repos-uri}/{gh-branch}/main

Regular aports that are not available in the official Alpine repository, typically some specific dependencies for _bundles_.
